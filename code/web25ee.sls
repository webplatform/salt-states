...
# rsync -a --no-perms --delete --filter '- .rsync-filter' --password-file=/etc/codesync.secret codesync@salt::code/web25ee/repo/ /srv/webplatform/web25ee/
# rsync --dry-run -a --no-perms --delete --filter '- /srv/code/web25ee/repo/.rsync-filter' --password-file=/etc/codesync.secret codesync@salt::code/web25ee/repo/ /srv/webplatform/web25ee/
# @salt-master-dest
sync-web25ee:
  cmd.run:
    - name: rsync -a --no-perms --password-file=/etc/codesync.secret codesync@salt::code/web25ee/repo/ /srv/webplatform/web25ee/
    - require:
...
/srv/webplatform/web25ee/backoffice/expressionengine/config/config.php:
  file.managed:
    - source: salt://code/files/web25ee/config.php.jinja
    - template: jinja
    - mode: 666
    - user: www-data
    - group: www-data
...
/srv/webplatform/web25ee/backoffice/expressionengine/config/database.php:
  file.managed:
    - source: salt://code/files/web25ee/database.php.jinja
    - template: jinja
    - mode: 666
    - user: www-data
    - group: www-data
